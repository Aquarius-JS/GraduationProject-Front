...
export default class Admin extends Controller {
  ...
  approveRegister = async (registerId, remark) => {
    const res = await this.fetch('/admin/approveRegister', {
      method: 'POST',
  ...
  violationInfoAppealHandle = async data => {
    const res = await this.fetch('/violationInfoAppealHandle', {
      method: 'POST',
      body: JSON.stringify(data),
    });
    return res;
